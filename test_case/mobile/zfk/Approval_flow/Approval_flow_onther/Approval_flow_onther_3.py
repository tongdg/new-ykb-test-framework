...
class Approval_flow_case(unittest.TestCase):
     ...
     def test_Approval_flow_case(self):
        #差旅报销单财务初审环节加审业务环节-----------------------------------------------------------------------------------------------------------------------
         ...
         self.zfk_Approvale_result = self.zfk_Approval_flow_onther_page.To_Approvel_travel()
         self.zfk_Approvale_result=self.zfk_Approval_flow_onther_page.add_Approvel_travel()
         self.zfk_Approvale_result=self.zfk_Approval_flow_onther_page.submit_travel_approver()
