...
class BillPage(IndexPage):
    ...
    def evection_date(self):
        return self.find_element_by_css_ykb('#tb > tbody > tr.row-1 > td.date.td-date.cell-value-1 > input:nth-child(2)')
    ...
    def evection_place_control(self):
        return self.find_element_by_hierarchy(lambda var : self.driver.find_element_by_css_selector("div[class='item']").find_elements_by_tag_name('a'))
    # 出差到达地点
    @property
    ...
    @property
    def travel_row(self):
        return self.find_element_by_hierarchy(lambda var : self.driver.find_element_by_css_selector('#traTable > tbody').find_elements_by_tag_name('tr'))
    # 现有单据的第N行的第几列
    ...
    def travel_row_colum_trip_explain(self,n,m):
        return self.find_element_by_hierarchy(lambda var : self.travel_row_column(n,m).find_element_by_tag_name('textarea'))
    # 填写城际交通  第二列
    ...
    def travel_intercity_traffic_tbody(self):
        return self.find_element_by_hierarchy(lambda var : self.driver.find_element_by_css_selector('#chjiList > table > tbody').find_elements_by_tag_name('tr'))
    # 城际交通填写页面的第N行的第几列
    def travel_intercity_traffic_row_column(self,n,m):
        return self.find_element_by_hierarchy(lambda var : self.travel_intercity_traffic_tbody[n].find_elements_by_tag_name('td')[m])
    # 特殊处理 金额的填写
    @property
    def travel_intercity_traffic_price(self):
        return self.find_element_by_hierarchy(lambda var : self.travel_intercity_traffic_row_column(0,5).find_element_by_css_selector("input[class='nobg text-right']"))
    # 城际交通确定按钮
    @property
    ...
    def travel_intercity_traffic_vehicle(self):
        return self.find_element_by_hierarchy(lambda var : self.driver.find_element_by_css_selector('#chjiList > table > tbody > tr:nth-child(1) > td.dpselect > div > div > div > ul').find_elements_by_tag_name('li'))
    # 城际交通地点控件
    @property
    def travel_intercity_traffic_palace_control(self):
        return self.find_element_by_hierarchy(lambda var : self.driver.find_element_by_css_selector('#app > div:nth-child(7) > table > tbody > tr:nth-child(1) > td:nth-child(2) > ul').find_elements_by_tag_name('li'))
    # 填写酒店住宿  第三列
    ...
    def cost_bill_abstract(self):
        return self.find_element_by_hierarchy(lambda var: self.cost_bill_row_column(1,1).find_element_by_tag_name('textarea'))
    # 金额特殊处理
    @property
    def cost_price(self):
        return self.find_element_by_hierarchy(lambda var: self.cost_bill_row_column(1,3).find_element_by_tag_name('input'))
    # 费用名称弹框
    @property
    ...
    def cost_name_list(self):
        return self.find_element_by_hierarchy(
            lambda var: self.driver.find_element_by_css_selector(
                '#app > div.bzBox3.noBod > table > tbody > tr:nth-child(3) > td:nth-child(2) > div.panelscoll').find_elements_by_tag_name('label'))
    """
       新单据通用
    """
    # 费用归属
    ...
    def new_cost_belong_row(self):
        return self.find_element_by_hierarchy(lambda var: self.driver.find_element_by_css_selector('#shareList > table > tbody').find_elements_by_tag_name('tr'))
    # 现有单据的第N行的第几列
    def new_cost_belong_row_column(self, n, m):
        return self.find_element_by_hierarchy(lambda var: self.new_cost_belong_row[n].find_elements_by_tag_name('span')[m])
    # 选择部门,选择项目
    @property
    ...
    def __fill_loan_bill(self):
        ...
        time.sleep(2) # 2S防止点击过快出错
        # 提交审批
        while self.driver.find_element_by_css_selector("button[class='eui-btn eui-btn-blue btn-commit']").is_displayed() is False:
            time.sleep(1)
        ...
        self.click(self.old_cofirm_button)
        # 等待带锯提交
        print(self.driver.find_element_by_css_selector("button[class='eui-btn eui-btn-blue btn-commit-confirm disabled']").is_displayed())
        self.wait_element_disappear_old_in_approval_btn()
        time.sleep(3) # 3S防止提单的暂时卡住，导致下次填单出错
        return True
