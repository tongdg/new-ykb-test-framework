...
class zfk_Approval_4_page(zfk_Mobile_index_page):
#差旅报销单-普通员工-提单- 审批领导-批单  会计-批单  财务经理批单- 出纳批单
    #存取单据号
    ...
    def cost_list(self):
        ...
        time.sleep(3)
        self.send_keys(self.find_element_by_css_ykb('#remarkMsg'), '差旅报销单测试说明')
        time.sleep(2)
        self.click(
            self.find_element_by_css_ykb(
    ...
    def cost_leader_approvel(self):
        ...
        if approval_order is not  False:
            ...
            self.click(
                self.find_element_by_css_ykb(
                    '#traForm > div > div.mui-bar.mui-bar-tab.ab > button.mui-btn.themebgStyle.mui-pull-right.themeBtn089a'
                )
            )
            time.sleep(2)
            self.log.info('[差旅报销单-审批领导-审批提单-成功]')
            time.sleep(3)
            self.click(
                self.find_element_by_css_ykb(
                    '#leader > div > div.mui-bar.mui-bar-tab.hasBLine.ab.spryanxs > button'
    ...
    def cost_accountant(self):
        ...
        self.click(self.find_element_by_css_ykb(
            '#guo > div.main.scroller > div > div:nth-child(2) > div > div.tab-main > div.tab-item.tabs-active > div > div:nth-child(2) > div.guo_scroll_content > div.form_item_ul > div'
            )
        )
        time.sleep(3)
        ...
        time.sleep(4)
        self.log.info('差旅报销单-财务初审环节-审批成功')
        time.sleep(2)
        return True
    #财务经理批单-出纳
    def cost_manager_account(self):
        ...
        self.click(
            self.find_element_by_css_ykb(
                '#guo > div.main.scroller > div > div.ykb_content > div > div.tab-tit > div:nth-child(1)'
            )
        )
        time.sleep(3)
        #点击-输入框
        ...
        time.sleep(4)
        self.log.info('差旅报销单-财务复核环节-审批成功')
        time.sleep(2)
        return True
    #差旅报销单-出纳-付款
