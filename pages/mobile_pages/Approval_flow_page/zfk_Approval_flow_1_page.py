...
import time
class zfk_Approval_1_page(zfk_Mobile_index_page):
#出差申请单-提单- 审批领导-批单
   ...
   # 选择机票Choince_plan
    def Choince_plan(self):
        ...
        Evection_standard = self.find_element_by_css_ykb(
            "#container > div > div.index-content > div.travel-standard > div.right").text
        ...
        plan_seat = random.randrange(1, 4)
        print('随机仓位' + str(plan_seat))
        time.sleep(5)
        self.click(
            self.find_element_by_xpath(
                '//*[@id="container"]/div/div[1]/div[2]/article/div[' + str(choince_plan) + ']/ul/li[' + str(
                    plan_seat) + ']/div[2]'

            )
        )
        time.sleep(4)
        plan_test=self.find_element_by_css_ykb('#app > div > header > h1')
        if plan_test is not False:
            # 点击提交按钮
            self.click(
                self.find_element_by_css_ykb(
    ...
    def leader_approvel(self):
        ...
        if approval_order is not  False:
            ...
            time.sleep(2)
            self.log.info('[-普通员工-出差申请单-选择机票提单-成功]')
            return True
        else:
            self.log.info('[--普通员提单至审批领导-失败--]')
            time.sleep(3)
            return False
        #判断流程-审批流程
